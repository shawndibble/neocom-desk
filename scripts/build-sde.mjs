...
async function main() {
  ...
  // --- functions/src/data/miningTypeIds.ts: every type_id the Mining Yield
  // Overview's server-side hub-price job (issue #1279, ADR TBD) needs a price
  // for — raw ore/ice, gas, every compressed counterpart, and every
  // reprocessing material a mined type refines into. `functions/` is a
  // separate package with no `public/data` access at deploy time (Cloud
  // Functions bundles only its own source tree), so this is baked as its own
  // small, committed file rather than read cross-package — the same reason
  // `compressedOreTypeIds.json` above is itself a precompute rather than a
  // name-match done at read time. A `.ts` source (a plain array literal)
  // rather than a `.json` file: `functions/`'s NodeNext module setup needs
  // either `resolveJsonModule` plus an import-attribute at every call site or
  // a runtime `readFileSync`/deploy-copy step to consume JSON, and a
  // generated TS literal sidesteps all of that for a file nothing but this
  // build script ever writes.
  const miningMaterialTypeIds = new Set();
  for (const typeID of [...oreAndIceTypeIds, ...gasCloudTypeIds]) {
    for (const material of reprocessing[typeID]?.materials ?? []) {
      miningMaterialTypeIds.add(material.typeID);
    }
  }
  const miningTypeIds = [
    ...new Set([
      ...oreAndIceTypeIds,
      ...gasCloudTypeIds,
      ...Object.values(compressedOreTypeIds),
      ...miningMaterialTypeIds,
    ]),
  ].sort((a, b) => a - b);
  ...
  const FUNCTIONS_DATA_DIR = join(ROOT, 'functions', 'src', 'data');
  await mkdir(FUNCTIONS_DATA_DIR, { recursive: true });
  {
    const ts =
      '// Generated by scripts/build-sde.mjs — do not hand-edit.\n' +
      `export const MINING_TYPE_IDS: readonly number[] = ${JSON.stringify(miningTypeIds)};\n`;
    await writeFile(join(FUNCTIONS_DATA_DIR, 'miningTypeIds.ts'), ts);
    console.log(
      `  functions/src/data/miningTypeIds.ts: ${Buffer.byteLength(ts).toLocaleString()} bytes`
    );
  }
  const outputs = [
    ['skills.json', skills],
