...
describe('item detail pane (issue #160)', () => {
  it('shows a hint until an item is selected, then shows its detail beside the list on selection', async () => {
    server.use(
      http.get(`https://esi.evetech.net/characters/${CHAR_ID}/assets`, () =>
        HttpResponse.json(
          [
    ...
    const itemButton = await screen.findByRole('button', { name: /Drake/ });
    await user.click(itemButton);

    const heading = await screen.findByRole('heading', { name: 'Drake' });
    const pane = within(heading.closest('section') as HTMLElement);
    expect(pane.getByText('92,150 m3')).toBeInTheDocument();
    expect(pane.getByText('1,200,000 ISK')).toBeInTheDocument();
    expect(document.querySelector('img[src*="/types/650/icon"]')).toBeInTheDocument();
    // Station + jumps-away, reused from whatever the route has already
    // resolved for Jita (issue #160 acceptance criteria) — this fixture's
    // character starts in the same system as the station, so it resolves
    // to "0 jumps" without a route call (issue #87).
    expect(pane.getByText(JITA)).toBeInTheDocument();
    expect(await pane.findByText('0 jumps')).toBeInTheDocument();
    expect(pane.getByText('0.9')).toBeInTheDocument();

    // Browsing the list is unaffected by the selection.
    expect(screen.getByRole('button', { name: /Drake/ })).toBeInTheDocument();
  });
  ...
  it('shows the volume as unknown for a type outside the slim SDE snapshot', async () => {
    ...
    const itemButton = await screen.findByRole('button', { name: /Mystery Module/ });
    await user.click(itemButton);

    expect(await screen.findByRole('heading', { name: 'Mystery Module' })).toBeInTheDocument();
    expect(screen.getByText('Unknown')).toBeInTheDocument();
  });

  it('shows one column at a time on narrow screens, with a back control that clears the selection', async () => {
