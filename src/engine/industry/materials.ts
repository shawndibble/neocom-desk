...
import { RIG_MATERIAL_BONUS_PCT, RIG_SECURITY_MULTIPLIER } from '@/engine/industry/types';
...
export function materialModifier(me: number, ctx: FacilityContext): number {
  ...
  // Rigs only work on player structures.
  const rigPct = ctx.facility.structure
    ? RIG_MATERIAL_BONUS_PCT[ctx.rig] * RIG_SECURITY_MULTIPLIER[ctx.security]
    : 0;
