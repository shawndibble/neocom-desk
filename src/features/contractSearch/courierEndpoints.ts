 ...
 * all.
 *
 * The price is that a player structure stays unnamed. `CourierEndpoint` says
 * so with `null` rather than guessing, and the table shows the raw id — the
 * honest answer, and one that still lets the route search match the *other*
 * end. Origin keeps its region regardless: the row carries the contract's own
 * region, which is where the pickup is.
 */
