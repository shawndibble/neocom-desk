...
export function ComputedQueue({ steps, nameFor, userSkillTypeIDs }: ComputedQueueProps) {
  const { t } = useTranslation();

  if (steps.length === 0) {
    return <EmptyState title={t('plans.computedQueueEmpty')} className="py-4" />;
  }
        ...
        return (
          ...
          >
            <span className="flex-1 truncate">
              ...
              {isPrereq && <span className="ml-2 text-[10px] uppercase">{t('plans.prereq')}</span>}
            </span>
            <span className="tabular-nums text-text-dim">{formatDuration(step.seconds)}</span>
            <span className="tabular-nums">{formatDuration(step.cumulativeSeconds)}</span>
          </li>
        );
      })}
    </ul>
