...
interface ComputedQueueProps {
  ...
  /**
   * When training begins, for the per-step start/finish line (#20). Omitted
   * (no timeline shown) when the caller has no wall-clock basis to offer —
   * e.g. a paused queue — rather than inventing one.
   */
  startDate?: Date;
}
    ...
    <div className="rounded-xs border border-line">
      ...
      <ul>
        {steps.map((step) => {
          ...
          const timeline = startDate ? stepTimeline(step, startDate) : null;
          return (
            <li
