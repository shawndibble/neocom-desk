...
describe('useAppraisal', () => {
  it('does not price anything until Appraise is pressed', async () => {
    const { result } = renderHook(() => useAppraisal(DEFAULT_TRADE_HUB, 90));
  ...
  it('prices the submitted text on Appraise', async () => {
    ...
    expect(appraisePaste).toHaveBeenCalledWith('Tritanium 5', DEFAULT_TRADE_HUB, 90, {
      force: false,
    });
  });
  ...
  it('re-prices on its own when the percentage changes', async () => {
    const { result, rerender } = renderHook(
      ({ percent }) => useAppraisal(DEFAULT_TRADE_HUB, percent),
    ...
    expect(appraisePaste).toHaveBeenLastCalledWith('Tritanium 5', DEFAULT_TRADE_HUB, 80, {
      force: false,
    });
  });
  ...
  it('re-prices when the hub changes', async () => {
    ...
    const { result, rerender } = renderHook(({ hub }) => useAppraisal(hub, 90), {
    ...
    rerender({ hub: amarr });
    await waitFor(() => expect(appraisePaste).toHaveBeenCalledTimes(2));
    expect(appraisePaste).toHaveBeenLastCalledWith('Tritanium 5', amarr, 90, { force: false });
  });
  ...
  it('bypasses the price cache on refresh, and only that once', async () => {
    ...
    await waitFor(() => expect(appraisePaste).toHaveBeenCalledTimes(2));
    expect(appraisePaste).toHaveBeenLastCalledWith('Tritanium 5', DEFAULT_TRADE_HUB, 90, {
      force: true,
    });
