...
/** The "Sold" split button wired to `useSaleLinking`, keyed off each row's own run/product — no plan context needed. Its Remove Run item deletes the row's run, so this column also carries the delete action for both tables. */
  ...
  return {
    ...
    render: (r) => (
      <SoldSplitButton
        ...
        onRemove={() => sale.confirmRemoveRun(r.run.id)}
        onRefresh={
