 ...
 *
 * Two effects and two memos, in that order: load every Character's assets once
 * per mount (cache-first — the shared ESI cache only calls out when its
 * freshness window has lapsed), count against the plan's material set, then
 * resolve names for just the locations the count surfaced.
 *
 * The aggregation memo is keyed on the asset snapshot and the *blueprint's*
 ...
 */
export function useDetectedOwnedStock(typeIDs: readonly number[]): DetectedOwnedStockResult {
