...
export async function loadFittingFromText(text: string): Promise<LoadFromTextResult> {
  ...
  if (input.kind === 'unknown') {
    return { ok: false, fitting: null, unresolved: [], error: 'unrecognised' };
  }
  ...
  if (input.kind === 'eft') {
    ...
    if (typeof loaded === 'string') {
      return { ok: false, fitting: null, unresolved: [], error: loaded };
    }
    result = loaded;
  }
  if (result.hullTypeId === null) {
    return { ok: false, fitting: null, unresolved: result.unresolved, error: null };
  }
