...
export function FilterChip({ label, selected, onToggle, count, className = '' }: FilterChipProps) {
  return (
    <button
      ...
      onClick={onToggle}
      className={`inline-flex h-7 items-center gap-1.5 rounded-xs border px-2.5 text-[0.6875rem] font-semibold tracking-widest uppercase transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent ${
        selected
