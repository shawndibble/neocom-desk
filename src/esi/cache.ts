...
export interface CachedResult<T> {
  ...
  fromCache: boolean;
  /**
   * The list is missing pages. Stored on the row, so it stays true for a
   * cache hit — a short list must not look whole after a reload.
   */
  truncated: boolean;
}
 ...
 *
 * The rule is enforced here rather than offered as an option: a partial list
 * may not overwrite a complete one. An older complete list under an honest
 * Data Age beats a fresh list that silently lost a page. A partial is still
 * stored when nothing is cached, or when what is cached is itself partial —
 * refusing outright would leave the cache permanently cold for an endpoint
 * that truncates every time, like the wallet transactions page cap.
 *
 * Every paginated endpoint returns `TruncatableResult`, and this is the only
 * cache entry point that accepts one, so a caller cannot silently drop the
 * flag the way an opt-in predicate allowed.
 */
export async function loadPaginatedWithCache<T>(
  ...
  try {
    ...
    // Keep the complete row; report it, not the short live one.
    return {
    ...
    // Offline/5xx/auth: fall back to whatever is cached, as loadWithCache does.
  }
  const cached = await readCachedRow(characterId, key);
