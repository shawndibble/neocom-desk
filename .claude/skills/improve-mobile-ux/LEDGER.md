...
- **Narrow-viewport specs in CI** — `e2e/charactersToolbarNarrow.spec.ts`,
  ...
  `{ width: 390, height: 844 }`. Industry, Mail, Mining Tax and Market's Open
  Orders still have none — the four tickets filed this round each add one
  (#1053/#1054/#1055/#1064).
...
- **Bare-button disclosure rows under the touch tier** — `#1064`'s fix is
  scoped to `OpenOrdersPanel`, but the hostile reviewer flagged that
  `src/components/ui/Disclosure.tsx` itself ships `min-h-8` (32px, under even
  the pointer floor) and that Skills/Market-Group-tree headers hand-roll the
  same bare-button pattern rather than using it — worth a dedicated pass
  rather than folding into #1064's one-file fix.
