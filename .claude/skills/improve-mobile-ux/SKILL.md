...
5. `gh issue list --state all --search "<terms>"` — already filed, open or
   ...
   before treating a hit as this skill's own prior art. An `/improve-ui`
   ticket on the same surface is prior art too: that skill runs this one
   alongside it, and each of its tickets states its phone behavior.
