...
research sub-agents and costs a great deal, so back-to-back sweeps are waste.
Seven runs in one day once produced two tickets and then four consecutive empty
ones, each re-reading forum pages an earlier run had read the same afternoon.

```
git -C <main-repo> log -1 --format=%cI origin/main -- .claude/skills/add-missing-features/TOOLS.md
```

- **Ledger touched within the last 5 days, and `$ARGUMENTS` is empty** → do
...
- **Ledger older than 5 days** → proceed with the full sweep below.

The cadence is a 5-day check for new ideas and apps. On top of that, after an
EVE expansion, **query that expansion's own mechanic nouns** within roughly six
months of release.
...
calls and a curated `TOOLS.md` update. The `TOOLS.md` update **does** need a
sibling worktree to commit from — never commit in the main checkout — see
step 7 for the exact branch/PR mechanics that keep concurrent runs from
colliding.
...
Check `git status -sb` for a `[behind N]` marker before trusting local
`grep`/`ls` results. A stale local checkout has caused real "proposed a
feature that shipped months ago" misses; if behind, verify against
`origin/main` (`git fetch origin main`, then `git show origin/main:<path>`)
instead of the stale working tree.
...
**The listing paginates** via `more_topics_url` in the response. Fetch at
least page 0 and page 1, and keep following the link while it's present —
several early runs only ever fetched page 0 and missed tools (EVE Horizon,
EVE Forge, EVE-HUB) that held the only surviving PI candidates found so far.
...
Also run `gh issue list --state all --search "created:<since-TOOLS.md-was-
last-updated>..<today>"` — search back to the ledger's last update, not just
today, since same-day runs (this skill's own, and `/next-ticket`'s and
`/improve-mobile-ux`'s) can file issues before any of them update the ledger. A
title alone doesn't say which skill filed a hit in a mixed batch — open the
body and check for the `> _This was generated by AI during
/add-missing-features._` provenance line before treating it as this skill's
own prior art.
...
## 7. Update the ledger — curate, don't append

[TOOLS.md](TOOLS.md) is a **reference for the next run**, not a run log.
Update it in place; never append a new dated section, and never record run
metadata (dates, commit-drift counts, "crashed before step 7", merge
post-mortems, agent housekeeping) — none of it helps the next run decide what
to propose. Keep the file under roughly 150 lines; when a section outgrows
that, compress or delete rather than let it grow.

Maintain these sections, each keyed by topic, not by run:

- **Tools surveyed** — one row per tool, deduplicated. Add new tools; never
  re-describe one already there.
- **Already covered** — grouped by area, each claim backed by the module or
  route that proves it.
- **Standing kill-tests** — reusable heuristics (cross-player aggregation
  wall, no write-scoped ESI, "picture of data already on screen", etc.). Add
  one only if it's genuinely reusable, not specific to a single candidate.
- **Filed candidates** — issue number, verdict, one-line description.
- **Killed / dropped candidates** — one line: what, why. This is what stops a
  re-pitch, so keep the reason, not the narrative.

### Commit it via one shared PR, not a new one per run

This skill runs unattended and concurrently — multiple same-day runs are
expected, and each one recording its own PR is exactly the failure mode this
section exists to prevent. Use a **fixed branch name** every time,
`chore/add-missing-features-ledger`, so "is a PR already open" is a
deterministic check rather than a title guess:

```
gh pr list --head chore/add-missing-features-ledger --state open --json number,url
```

- **A PR is already open**: branch a sibling worktree off that branch (never
  the main checkout), rebase onto `origin/main`, merge your curated update
  into the existing content — not a second append — commit, and push to the
