...
| EVE Motor Market: cross-plan material reservation ("freeze allocated stock so you never buy materials twice")                    | Settled scope — `20260909-212724-group-ownership-overlay-...md` explicitly rejected a cross-plan allocation solver ("No allocation problem to solve... which member 'gets' a shared unit of a material several plans need") and retired `groupRollup.ts`'s own `overClaimed` double-counting flag once Group Owned Overlay shipped. A Build Group is the app's answer to shared ownership across plans; extending it to arbitrary ungrouped plans reopens exactly the problem that decision closed. |
