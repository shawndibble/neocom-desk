...
| Assets (seeded 3 locations, location item list)                                | Chrome clean; the tall single-location panel is a scroll container, not a dead zone. Item rows print volume with no unit and rounded to 0: #1995.                                                                                                                                                                                                                                             |
...
| Contacts (12-24 seeded), Loyalty Store (14 offers), Clones, Employment History | Contacts: header info icon drifts from its label (shared `DataTable`): #1953. Loyalty Store: narrow list plus empty detail pane is the unselected state; clean. Clones empty state clean.                                                                                                                                                                                                     |
...
- **Override ESI by host and exact path.** A `page.route` regex without the `esi.evetech.net` host also matches the app's own module URLs and blanks the page.
...
| #1976 | tweak | NARROW   | Contacts: Labels column from the contact-labels endpoint (filter group deferred).            |
| #1995 | tweak | NARROW   | Assets item rows: volume with an m³ unit and sub-1 precision (bug).                          |
...
- Clones: pin an Active clone row (implants from `/implants`, location from `/location`). Skills already shows the active implants, `/location` is a new scope and a second `GrantBanner` branch on a one-scope page, and `DataTable` has no pinned row. Cost too small for the size.
- Assets: sort items by value or quantity inside a location. Cut by review: name-only sorting there is documented in `sortNodes` and `assetSortPreference.ts`, the flat All Items view sorts by value, and container sorting needs an aggregate-value decision. Re-pitch only as `ready-for-human` with that decided.
