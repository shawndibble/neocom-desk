...
- **Sync is last-write-wins on the whole record**, so a device running a build from before notes existed, saving a fitting later than the notes were written, replaces the record without them. A newer build's notes are not merged per field.
