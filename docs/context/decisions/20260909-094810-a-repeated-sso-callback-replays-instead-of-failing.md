...
- **A callback that lands twice is a success, not a failure.** `completeLogin`
  ...
  of throwing. #649's report is a switch-user round trip on EVE's login page
  ending on a "Login failed" panel; the one shape that produces an error over a
  session that in fact exists is a second landing on the same
  `/callback?code=…&state=…`, because the first landing already spent the
  one-shot stash. `Callback.tsx`'s `started` ref cannot cover it — a ref spans
  one mount, and a second landing is a fresh page load.
