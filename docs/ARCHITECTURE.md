...
| `src/engine` (+ `optimizer`, `industry`, `import`) | **Pure** calculation: `types.ts` (engine-native shapes, decoupled from SDE/ESI), `plan.ts`/`schedule.ts`/`sp.ts` (training-time math), `clipboardExport.ts`/`queueImport.ts`, `optimizer/{bestAttributes,placeRemaps,reorderSuggestion,optimizeAtMarkers}` (remap placement, reorder, marker-delimited segment optimization), `industry/{buildVsBuy,fees,jobCost,materials,time,types}` (manufacturing math), `import/{eftFit,fitToSkills,skillPlanPaste}` (clipboard parsers)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **No fetch/DOM/Dexie/Firebase imports** (CLAUDE.md rule; TDD required). Callers adapt SDE/ESI shapes to engine types at the boundary | `features/skills`, `features/industry`                                   |
| `src/esi`                                          | `client.ts` (`esiFetch`: token injection via `configureEsi`, `X-Compatibility-Date`/`X-User-Agent`, single 429/420 retry, `EsiError`, `isAuthFailure`), `endpoints.ts` (one typed wrapper per ESI route), `paginated.ts`, `registry.ts` (single source of truth: every endpoint's required scope, consumed by `scopes.ts` and `app/routeScopes.ts`; also carries each route template, kept only for `registry.test.ts`'s marker-comment parity check — no runtime consumer yet; `satisfies Record<EndpointName, ...>` makes a missing entry a compile error), `scopes.ts` (read-only `SCOPES`, **derived** from `registry.ts` — do not hand-maintain), `cache.ts` (shared read-through cache: `loadWithCache`/`loadWithCacheStatus`, `GLOBAL_CACHE_CHARACTER_ID`, raw `readCached`/`writeCached` for batch callers), `cachePurge.ts` (escalating purge on scope revoke / owner change — targeted range delete → full table clear → in-memory + durable suppression; **never throws**) | Hold auth state beyond the injected token provider                                                                                   | every `features/*` data module                                           |
| `src/features/character`                           | Per-view ESI + cache for Wallet/Assets/Mail/Calendar/Contracts/Orders, all thin wrappers over `esi/cache`: `assets.ts`, `calendar.ts`, `contracts.ts`, `mail.ts`, `orders.ts`, `wallet.ts`, `names.ts` (entity name resolution), `typeNames.ts` (item type names), `stations.ts`, `format.ts`. Plus two cross-view helpers: `useRouteSnapshot.ts` (route lifecycle — hydration wait, load-on-refresh, stale-response discard) and `roster.ts` (all Characters at once; cache-only by default, capped-concurrency live mode)                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Duplicate calculation that belongs in `engine`                                                                                       | `routes/{Wallet,Assets,Mail,Calendar,Contracts,Orders}.tsx`              |
...
| `src/lib`                                          | Small shared utilities with no other natural home. Pure: `duration.ts` (`formatDuration`, "Xd Yh Zm"), `isk.ts` (`formatIsk(value, decimals = 0)` — clamps float noise so a near-zero value never renders `-0`), `csv.ts` (RFC 4180 + BOM + formula-injection guard), `rankedSearch.ts` (typeahead ranking shared by Market search and the Skill Picker), `concurrency.ts` (`ESI_FANOUT_CONCURRENCY` + `mapWithConcurrencyLimit` — one fan-out policy, counted in requests), `eveImages.ts` (`characterPortraitUrl`/`typeIconUrl` — every images.evetech.net URL; `size` is required because three earlier copies had drifted on their defaults). Not pure, and deliberately here rather than in a new directory: `download.ts` (DOM — triggers a file download) and `useLocalSetting.ts` (`createLocalSetting` — zustand + Dexie factory for one device-local, never-synced preference key; the `sync.` prefix is rejected)                                                          | Feature-specific logic (that belongs in the feature or `engine`); anything synced (that is `setSyncedSetting`)                       | `features/{character,industry,market,skills}`, `routes`, `components/ui` |
...
| `src/routes`                                       | One file per page: `Login`, `Callback`, `Characters`, `Overview` (+ `overviewQueue.ts`), `Skills`, `SkillPlans`, `Industry`, `Market`, `Wallet`, `Assets`, `Mail`, `Calendar`, `Contracts`, `Orders`, `Settings`, `Styleguide`. `Settings` is routable but deliberately absent from `Layout`'s nav until it holds a real control                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Own reusable logic other routes need — push into `features`/`engine`                                                                 | `app/App.tsx` (route table) only                                         |
...
| `src/stores`                                       | Zustand stores: `activeCharacter.ts` (persisted via Dexie `settings`, `hydrate`/`setActiveCharacter`), `publicInfo.ts` (session-only corp/alliance name cache, not durable), `authFailure.ts` (session-only runtime auth-failure sink — `esi/cache.ts` reports here so one shell notice covers every view)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Persist editable data outside `src/db`/sync                                                                                          | `app`, `routes/Characters.tsx`                                           |
| `src/styles`                                       | `index.css` — Tailwind v4 `@theme` tokens per `docs/DESIGN.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Component logic                                                                                                                      | build pipeline                                                           |
| `src/sync`                                         | `index.ts` (public barrel — **import from `@/sync` only**; it is also the code-splitting boundary: status/uid re-exports are static, every Firebase-touching call is a thin wrapper over `await import(...)`, keeping Firebase out of the entry chunk), `status.ts` + `uid.ts` (Firebase-free, safe to import synchronously), `planSync.ts` (driver: `triggerSync`/`scheduleSync`/status/`markPlanDeleted`/`markBuildPlanDeleted`/`setSyncedSetting`), `merge.ts` (pure LWW+tombstone merge, no Firebase import, unit-testable), `syncAuth.ts` (`ensureSignedIn` → `mintFirebaseToken` exchange), `firebaseApp.ts` (lazy Firebase app/auth/firestore/functions getters; uses **`firebase/firestore/lite`** — no `onSnapshot`, no offline persistence, which is correct because Dexie is the authoritative local store)                                                                                                                                                                | UI wiring (owned by `src/app`)                                                                                                       | `app/App.tsx`, `routes/Industry.tsx`, `features/skills/planner`          |
...
| `/skills/plans`                               | Skill Plans: editor, computed queue, clipboard import/export, optimizer (remap placement, reorder suggestion, optimize at markers), what-if implants/booster, **Remap Markers** (user-placed drag markers, CONTEXT.md round 2) | shipped           |
...
| `/settings`                                   | Device-local preferences. Routable but deliberately unlisted in nav until it holds a real control (item 18).                                                                                                                   |
