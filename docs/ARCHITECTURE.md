...
- **Editable** (Skill Plans, Build Plans, `sync.`-prefixed settings):
  ...
  Function, `mintFirebaseToken`). Firebase exists *only* for this sync path —
...
| `src/esi` | `client.ts` (`esiFetch`: token injection via `configureEsi`, `X-Compatibility-Date`/`X-User-Agent`, single 429/420 retry, `EsiError`, `isAuthFailure`), `endpoints.ts` (one typed wrapper per ESI route), `paginated.ts`, `scopes.ts` (read-only `SCOPES` list) | Cache to Dexie itself (that's `features/*/cache.ts`/`data.ts`); hold auth state beyond the injected token provider | every `features/*` data module |
| `src/features/character` | Per-view ESI + cache for Wallet/Assets/Mail/Calendar/Contracts/Orders: `assets.ts`, `calendar.ts`, `contracts.ts`, `mail.ts`, `orders.ts`, `wallet.ts`, `cache.ts` (shared read-through cache), `names.ts` (entity name resolution), `typeNames.ts` (item type names), `stations.ts`, `format.ts` | Duplicate calculation that belongs in `engine` | `routes/{Wallet,Assets,Mail,Calendar,Contracts,Orders}.tsx` |
| `src/features/industry` | Build Plan feature: `blueprintCatalog.ts`, `data.ts` (owned blueprints + jobs cache), `jobs.ts` (`ActiveJobsPanel` data), `computeBuildPlan.ts` (wires a `BuildPlanRecord` + blueprint + market data into `engine/industry/buildVsBuy`), `marketData.ts`, `duration.ts`, plus `*.tsx` panel components | Reimplement math already in `engine/industry` | `routes/Industry.tsx` |
...
`needsReauth: true` instead of silently going stale (`isAuthFailure` in
`esi/client.ts`). **This pattern is copy-pasted, not shared**, across
`features/skills/data.ts`, `features/character/cache.ts`,
`features/industry/data.ts` and `jobs.ts`, `features/character/names.ts` and
`typeNames.ts` — six independent copies. Consolidation into one shared module
is pending; don't add a seventh without checking whether this is finally the
time to unify them.
...
3. Add a per-view data module under `src/features/<area>/` using the read-through cache pattern (see §3) — check first whether the pending cache consolidation has landed before copy-pasting an eighth copy.
