...
- **BPC Sourcing**: Industry's third tab (Build Plans / Records / BPC Sourcing). Search over every publicly contracted Blueprint Copy for sale across all of New Eden — item type, region, ME/TE, runs, price (issue #608, ADR 0013). Not per-Character: a scheduled backend job crawls EVE Ref's public-contracts dataset (ESI itself has no search over public contracts) and republishes the shared, read-only **Public Contract Offers snapshot** every 30 minutes for every signed-in Character to search. That snapshot carries every item type; BPC Sourcing narrows it to blueprint copies in the browser (issue #907), rather than the crawl narrowing it for this one search. Distinct from the personal **Contracts** view, which is one Character's own issued/accepted contracts read straight from ESI.
