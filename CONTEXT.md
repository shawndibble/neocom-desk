...
- **BPC Sourcing**: Industry's third tab (Build Plans / Records / BPC Sourcing). Search over every publicly contracted Blueprint Copy for sale across all of New Eden — item type, region, ME/TE, runs, price (issue #608, ADR 0013). Not per-Character: a scheduled backend job crawls EVE Ref's public-contracts dataset (ESI itself has no search over public contracts) and republishes the shared, read-only **Public Contract Offers snapshot** every 30 minutes for every signed-in Character to search. That snapshot carries every item type; BPC Sourcing narrows it to blueprint copies in the browser (issue #907), rather than the crawl narrowing it for this one search. Distinct from **Contract Search**, the Contracts page's own tab over the same snapshot unnarrowed, and from the History tab beside it, which is one Character's own issued/accepted contracts read straight from ESI.
- **Offer** (BPC Contract Search): one for-sale row of that snapshot — in BPC Sourcing, a single blueprint copy listing at a single price. Not a **copy**: one contract can offer `quantity: 3` copies at one price, so a count of offers is smaller than a count of copies. Every count on the search says "offers", because an offer is what a buyer chooses between.
...
- **Compare**: A resizable bottom drawer over the **Compare Set**, not a tab
  ...
  **Appraisal**'s **Compare hubs**, which prices one pasted pile across the
  five trade hubs rather than several items against each other, and from
  **Fitting Compare**, the Fittings section's own page, which never touches
  the Compare Set.
...
- **ISK/run**: What a listed blueprint copy costs for each use it carries —
  ...
  on the Courier board: a derived per-unit rate over columns the table already
  holds, sortable, additive. Deliberately _not_ the ranking figure — the panel
  still sorts on price, the "cheapest listed offer" framing a buyer arrives
  with. Quantity divides as well as runs: an **Offer** is a contract row, not a
  copy, so a listing of three ten-run copies is one indivisible purchase of
  thirty runs. A row with no asking price — an owned blueprint — and a copy
  stating zero runs have no ISK/run at all; both show the unavailable marker
  and sort last, never an infinite rate.
...
- **Projection Horizon**: How far ahead a Projection reaches — 72 hours. A
  ...
  server-side.
- **Public Contract Offers snapshot**: the shared, read-only Firestore collection (`publicContractOffers`) holding every for-sale line of every public item_exchange/auction contract, any item type — the same twice-hourly EVE Ref crawl behind **BPC Sourcing**, generalized past blueprint copies (issue #906). "For sale" is the whole of it: a line the contract issuer is _asking_ for is not in the snapshot. It has two readers: **BPC Sourcing** narrows it to blueprint copies in the browser — which is what let the blueprint-only `publicBpcContracts` collection and its sync be retired (issue #907) — and **Contract Search** takes it whole (issue #908). The two read and cache it separately rather than sharing one full-size entry. It covers only the contract types that sell an item; the **Public Courier Contracts snapshot** is its sibling for the ones that move one.
- **Public Courier Contracts snapshot**: the shared, read-only Firestore collection (`publicCourierContracts`) holding every outstanding public courier contract as a route and a fee — origin, destination, reward, collateral, volume and days-to-complete (issue #909). Published by the same twice-hourly crawl and the same scheduled function as the **Public Contract Offers snapshot**, off the same single archive fetch; a sibling collection only because a courier contract has no item lines and so none of an offer row's `typeId`/`price`/`quantity`. A row is published only when the archive states all four of those endpoints, reward and volume: a _blank_ CSV column converts to `0` rather than to nothing, which would publish a delivery to station 0 or a free haul indistinguishable from a stated one. A stated `0` is kept, since a favour run really does pay nothing. ESI populates all four on courier contracts, so that drop is a guard against a schema change rather than an expected filter — the sync logs outstanding courier contracts beside kept rows so a guard that starts firing is visible. **Contract Search**’s Courier mode is its one reader (issue #910), which names each end out of the local `stations.json`/`systems.json` snapshots rather than ESI — so a player structure stays an unnamed id rather than costing a request per haul.
