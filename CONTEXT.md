...
## Scope decisions (round 38) — the corp ops board
...
- **`canReadMoonExtractions` is its own Corp Capability**, though it shares
  ...
  in ESI's spec.)
- **A corp 403 never raises the app-wide auth-failure notice.** The corp
  loaders override `detectAuthFailure` to `false`, which is also what
  suppresses `emitEsiAuthFailure` — a revoked role must degrade quietly, not
  herd the user toward a re-login that cannot restore it.
