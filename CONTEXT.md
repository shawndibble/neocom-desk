...
- **What-If Implants**: Optimizer override that assumes a hypothetical implant set instead of the clone's current implants. Five independent per-attribute bonuses (+0..+5 each), since EVE's attribute hardwirings are per slot — a clone can run +4 PER / +5 INT / +3 MEM and nothing in WIL or CHA. The matched sets (+1..+5 in every slot) remain one-click **presets** over those five values; see round 28. Stored on the Skill Plan and synced with it: it is one of the two lenses the plan's numbers are costed under, so a plan reopened without it quotes different training times than the ones its owner left it showing.
- **Booster**: Cerebral accelerator; user toggles it on manually with an expiry date for training-time math. Stored on the Skill Plan and synced with it, like What-If Implants above; the expiry is kept as an instant, not as wall-clock text, so it names the same moment on a device in another timezone.
...
- **A detected accelerator is modelled as a Booster, prefilled and editable —
  ...
  untouched, and is theirs to change or clear afterwards.
