...
- **Often Undercut**: The Open Orders badge for an order that reads `healthy` right now but has been undercut most of the times this page has watched it — `wasFrequentlyUndercut` in `src/engine/market/orderProblemHistory.ts`, over samples the page stores itself in `db.orderProblemSamples`. It is an `OrderBadgeKind`, never an **Order Problem**: the row stays in the healthy group, so nothing already reading worse is affected. Deliberately a boolean with no count beside it — the samples are taken only while this page is open, so any figure would overstate the precision. Silent until there are enough samples to judge.
