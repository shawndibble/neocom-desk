...
- **Public Courier Contracts snapshot**: the shared, read-only Firestore collection (`publicCourierContracts`) holding every outstanding public courier contract as a route and a fee — origin, destination, reward, collateral, volume and days-to-complete (issue #909). Published by the same twice-hourly crawl and the same scheduled function as the **Public Contract Offers snapshot**, off the same single archive fetch; a sibling collection only because a courier contract has no item lines and so none of an offer row's `typeId`/`price`/`quantity`. A row is published only when the archive states all four of those endpoints, reward and volume: a _blank_ CSV column converts to `0` rather than to nothing, which would publish a delivery to station 0 or a free haul indistinguishable from a stated one. A stated `0` is kept, since a favour run really does pay nothing. ESI populates all four on courier contracts, so that drop is a guard against a schema change rather than an expected filter — the sync logs outstanding courier contracts beside kept rows so a guard that starts firing is visible. Ingestion only as of #909: nothing in the client reads it yet.
